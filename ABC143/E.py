...
T = [0]*Q
for q in range(Q):
    S[q], T[q] = list(map(int, input().split()))
    S[q] -= 1
    T[q] -= 1
...
def main(): 
    for q in range(Q):
        #print(dijkstra_back(S[q], T[q], N, M, cost))
        root = dijkstra_back(S[q], T[q], N, M, cost)
        fuel = L
        res = 0
        for n in range(len(root)-1):
            fuel -= cost[root[n]][root[n+1]]
            if fuel < 0:
                fuel = L - cost[root[n]][root[n+1]]
                if fuel < 0:
                    print(-1)
                    break
                res+=1
