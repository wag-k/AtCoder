...
N, B[0], B[1], B[2] = list(map(int, input().split()))
l = [[0]*N for n in range(N)]
r = [[0]*N for n in range(N)]
...
def main():
    res = copy.deepcopy(l)
    for n in range(N):
        t = copy.deepcopy(res[n])
        for m in range(1,N):
            t[m] += t[m-1]
            if t[m] < B[2] and B[2] <= t[m]+(r[n][m]-l[n][m]):
                res[n][m] += B[2] - t[m]
                t[m] = res[n][m]
            elif B[2] < t[m]:
                t[m] = res[n][m]
            else:
                if t[m] < B[1] and B[1] <= t[m]+(r[n][m]-l[n][m]):
                    res[n][m] += B[1] - t[m]
                    t[m] += B[1] - t[m]
                else:
                    if t[m] < B[0] and B[0] <= t[m]+(r[n][m]-l[n][m]):
                        res[n][m] += B[0] - t[m]
                        t[m] += B[0] - t[m]
