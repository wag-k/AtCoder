...
def main():
    ...
    if len(B_list) ==0 or len(W_list)==0:
        print(0)
        sys.exit()
    num_BL =np.inf
    num_WR =np.inf
    res = np.inf
    for i, n in enumerate(BW_list):
        num_BL = len(B_list[:find_index(BW_list, n)+1])
        num_WR = len(W_list[find_index(BW_list, n):])
        res = min(res, num_BL+num_WR)
    print(res)
